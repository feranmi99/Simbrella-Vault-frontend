import { ArrowUpRight, ArrowDownLeft, Plus, Phone, PieChart, Zap } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {actions.map(({ id, title, subtitle, icon: Icon, bgColor, hoverBgColor, iconColor, hoverBorderColor, hoverBgButton }) => (
          <Button
            key={id}
            variant="outline"
            className={`flex flex-col items-center p-4 h-full transition group ${hoverBorderColor} ${hoverBgButton}`}
          >
            <div className={`p-3 rounded-full mb-3 transition ${bgColor} ${hoverBgColor}`}>
